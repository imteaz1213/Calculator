...
export default function AdvancedCalculator() {
  ...
  const [integrationLimits, setIntegrationLimits] = useState({ a: 0, b: 2, n: 6 });
  const [eulerParams, setEulerParams] = useState({ x0: 0, y0: 1, h: 0.2, xEnd: 1 });
  ...
  const calculate = () => {
    ...
    try {
      if (method === "gaussian" || method === "pivoting" || method === "lu") {
        const A = JSON.parse(matrixInput).map((row: any) => row.map((val: any) => parseFloat(val)));
      ...
      setResult(null);
      setIterationSteps([err.message || "Error: Please check your inputs!"]);
    }
  };
  ...
  const gaussianElimination = (matrix: number[][]) => {
    ...
    const logs: string[] = ["⚙️ Starting Gaussian Elimination...", `Initial Matrix: ${JSON.stringify(matrix)}`];

    for (let i = 0; i < n; i++) {
      // Partial Pivoting integrated for stability
      let maxRow = i;
      for (let k = i + 1; k < n; k++) {
      ...
      for (let k = i + 1; k < n; k++) {
        ...
        logs.push(`▶️ Row Op: R${k+1} = R${k+1} - (${factor.toFixed(4)}) * R${i+1}`);
        for (let j = i; j <= n; j++) {
    ...
    for (let i = n - 1; i >= 0; i--) {
      ...
      logs.push(`✍️ Step x${i+1}: ${stepExpr}`);
    }
  ...
  const partialPivoting = (matrix: number[][]) => {
    const n = matrix.length;
    const logs: string[] = ["⚙️ Analyzing Partial Pivoting layout...", `Original State: ${JSON.stringify(matrix)}`];
    for (let i = 0; i < n; i++) {
      let maxRow = i;
      for (let k = i + 1; k < n; k++) {
        if (Math.abs(matrix[k][i]) > Math.abs(matrix[maxRow][i])) maxRow = k;
      ...
      if (maxRow !== i) {
        ...
        logs.push(`✔️ [Iteration ${i+1}]: Row ${i+1} is already scaled with maximum coefficient (${matrix[i][i]}). No swap required.`);
      }
    }
    setIterationSteps(logs);
  ...
  const luDecomposition = (matrix: number[][]) => {
    ...
    const logs: string[] = ["⚙️ Starting Crout/Doolittle LU Decomposition..."];
  ...
  const simpson13 = (expr: string, a: number, b: number, n: number) => {
    ...
    const h = (b - a) / n;
    const logs: string[] = ["⚙️ Simpson's 1/3 Rule Quadrature Math:", `Step size (h) = (${b} - ${a}) / ${n} = ${h.toFixed(4)}`];
  ...
  const simpson38 = (expr: string, a: number, b: number, n: number) => {
    ...
    const h = (b - a) / n;
    const logs: string[] = ["⚙️ Simpson's 3/8 Rule Quadrature Math:", `Step size (h) = (${b} - ${a}) / ${n} = ${h.toFixed(4)}`];
  ...
  const eulerMethod = (expr: string, x0: number, y0: number, h: number, xEnd: number) => {
    ...
    let x = x0;
    let y = y0;
    const logs: string[] = ["⚙️ Initiating Euler's Method Iterative Solver: Formula: y(n+1) = y(n) + h * f(x, y)"];
      ...
      <div className="w-full max-w-3xl rounded-2xl bg-white p-8 shadow-xl dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700">
        ...
        <div className="mb-6">
          ...
          <select
            ...
            className="w-full p-3 rounded-lg border border-gray-300 dark:border-zinc-600 bg-gray-50 dark:bg-zinc-700 font-semibold"
          >
          ...
          {["gaussian", "pivoting", "lu"].includes(method) && (
            <div>
              ...
              <input
                ...
                className="w-full p-3 rounded-lg border font-mono text-sm bg-gray-50 dark:bg-zinc-700 border-gray-300 dark:border-zinc-600"
              />
            </div>
          )}
          ...
          {["simpson13", "simpson38"].includes(method) && (
            <div className="space-y-3">
              <div>
                ...
                <input type="text" value={functionInput} onChange={(e) => setFunctionInput(e.target.value)} className="w-full p-2 rounded border bg-gray-50 dark:bg-zinc-700 border-gray-300 dark:border-zinc-600 font-mono" />
              </div>
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block text-sm">Lower Limit (a):</label>
                  <input type="number" value={integrationLimits.a} onChange={(e) => setIntegrationLimits({ ...integrationLimits, a: parseFloat(e.target.value) })} className="w-full p-2 rounded border bg-gray-50 dark:bg-zinc-700 border-gray-300 dark:border-zinc-600" />
                </div>
                <div>
                  <label className="block text-sm">Upper Limit (b):</label>
                  <input type="number" value={integrationLimits.b} onChange={(e) => setIntegrationLimits({ ...integrationLimits, b: parseFloat(e.target.value) })} className="w-full p-2 rounded border bg-gray-50 dark:bg-zinc-700 border-gray-300 dark:border-zinc-600" />
                </div>
                <div>
                  <label className="block text-sm">Intervals (n):</label>
                  <input type="number" value={integrationLimits.n} onChange={(e) => setIntegrationLimits({ ...integrationLimits, n: parseInt(e.target.value) })} className="w-full p-2 rounded border bg-gray-50 dark:bg-zinc-700 border-gray-300 dark:border-zinc-600" />
                </div>
              </div>
              <div>
                <label className="block mb-1 font-medium text-amber-500">True Analytical Exact Value (For Error Analysis):</label>
                <input type="number" placeholder="Optional - Enter exact math integration answer" value={exactValue} onChange={(e) => setExactValue(e.target.value)} className="w-full p-2 rounded border bg-gray-50 dark:bg-zinc-700 border-gray-300 dark:border-zinc-600" />
              </div>
            </div>
          )}
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-4 gap-2">
                <div>
                  ...
                  <input type="number" value={eulerParams.x0} onChange={(e) => setEulerParams({ ...eulerParams, x0: parseFloat(e.target.value) })} className="w-full p-2 rounded border bg-gray-50 dark:bg-zinc-700 border-gray-300 dark:border-zinc-600" />
                </div>
                <div>
                  <label className="block text-xs">y0:</label>
                  <input type="number" value={eulerParams.y0} onChange={(e) => setEulerParams({ ...eulerParams, y0: parseFloat(e.target.value) })} className="w-full p-2 rounded border bg-gray-50 dark:bg-zinc-700 border-gray-300 dark:border-zinc-600" />
                </div>
                <div>
                  <label className="block text-xs">h (Step):</label>
                  <input type="number" step="0.01" value={eulerParams.h} onChange={(e) => setEulerParams({ ...eulerParams, h: parseFloat(e.target.value) })} className="w-full p-2 rounded border bg-gray-50 dark:bg-zinc-700 border-gray-300 dark:border-zinc-600" />
                </div>
                <div>
                  <label className="block text-xs">Target X:</label>
                  <input type="number" value={eulerParams.xEnd} onChange={(e) => setEulerParams({ ...eulerParams, xEnd: parseFloat(e.target.value) })} className="w-full p-2 rounded border bg-gray-50 dark:bg-zinc-700 border-gray-300 dark:border-zinc-600" />
                </div>
              </div>
            </div>
          )}
          ...
          <div className="mt-6 p-4 rounded-xl bg-gray-100 dark:bg-zinc-900 border border-gray-300">
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto pr-2">
              {iterationSteps.map((step, idx) => (
                <div key={idx} className="p-3 rounded bg-white dark:bg-zinc-800 text-xs font-mono border border-gray-200 dark:border-zinc-700 shadow-sm break-all leading-relaxed">
